...
static void read_from_mpl(void)
{
    ...
    MPU_DEBUG_FUNC();//调试函数（自己写的不是官方包含的）
		//inv_get_sensor_type_quat 获取四元素
    if (inv_get_sensor_type_quat(data, &accuracy, (inv_time_t*)&timestamp)) {
        ...
        */
		...
		//inv_get_sensor_type_accel 获取加速度
    if (hal.report & PRINT_ACCEL) {
        if (inv_get_sensor_type_accel(data, &accuracy,
            (inv_time_t*)&timestamp))
            eMPL_send_data(PACKET_DATA_ACCEL, data);
    }
		//inv_get_sensor_type_gyro  获取角速度
			  ...
			  if (inv_get_sensor_type_euler(data, &accuracy,(inv_time_t*)&timestamp))
						{
							...
							Pitch =data[0]*1.0/(1<<16);//inv_get_sensor_type_euler读出的数据是Q16格式，所以右移16位.
							Roll = data[1]*1.0/(1<<16);//inv_get_sensor_type_euler读出的数据是Q16格式，所以右移16位.
							Yaw = data[2]*1.0/(1<<16);//inv_get_sensor_type_euler读出的数据是Q16格式，所以右移16位.
							...
							Send_Data((int16_t *)&sensors.gyro.raw,(int16_t *)&sensors.accel.raw);
								...
								sprintf ( cStr, "Pitch :   %.4f   ",Pitch );	
								ILI9341_DispString_EN(30,90,cStr);

								sprintf ( cStr, "Roll :   %.4f   ", Roll );	
								ILI9341_DispString_EN(30,110,cStr);

								sprintf ( cStr, "Yaw :   %.4f   ", Yaw );	
				...
				if (timestamp > hal.next_pedo_ms) {
						...
						dmp_get_pedometer_step_count(&step_count);//获取步行数目
						dmp_get_pedometer_walk_time(&walk_time);//获取步行时间的持续时间。
...
int main(void)
{
	...
	Init_ILI9341();          //LCD 初始化
  ILI9341_GramScan ( 6 );
	LCD_SetColors(0x4e99,WHITE);
#endif
	//开启系统时钟
	SysTick_Init();
...
//调用 inv_init_mpl 函数初始化 MPL 软件库，初始化后才能正常进行解算
	  ...
	  /* Compute 6-axis and 9-axis quaternions. */
		//设置各种运算参数
    inv_enable_quaternion();//四元数运算
    inv_enable_9x_sensor_fusion();//6 轴或 9 轴数据融合
     ...
     */
    inv_enable_fast_nomot();//无运动状态时更新陀螺仪
    /* inv_enable_motion_no_motion(); */
    /* inv_set_no_motion_time(1000); */

    /* Update gyro biases when temperature changes. */
    inv_enable_gyro_tc();// 当温度变化时更新 陀螺仪
...
#else
    mpu_set_sensors(INV_XYZ_GYRO | INV_XYZ_ACCEL);//设置传感器的工作模式
#endif
    /* 把陀螺仪及加速度数据放进 FIFO. */
    mpu_configure_fifo(INV_XYZ_GYRO | INV_XYZ_ACCEL);
    mpu_set_sample_rate(DEFAULT_MPU_HZ);//采样率
#ifdef COMPASS_ENABLED
 ...
 * 1. 调用 dmp_load_motion_driver_firmware().
 它会把 inv_mpu_dmp_motion_driver.h 文件中的 DMP 固件写入到 MPU 的存储空间
 * 2. 把陀螺仪和加速度的原始数据矩阵送入 DMP.
 * 3. 注册姿态回调函数. 除非相应的特性使能了，否则该回调函数不会被执行
 * 4. 调用 dmp_enable_feature(mask) 使能不同的特性.
 * 5. 调用 dmp_set_fifo_rate(freq) 设置 DMP 输出频率.
 * 6. 调用特定的特性控制相关的函数.
 *
 * 调用 mpu_set_dmp_state(1)使能 DMP. 该函数可在 DMP 运行时被重复调用设置使能或关闭
 *
 * 以下是 inv_mpu_dmp_motion_driver.c 文件中的 DMP 固件提供的特性的简介：
 * DMP_FEATURE_LP_QUAT: 使用 DMP 以 200Hz 的频率产生一个只包含陀螺仪的四元数数据
 以高速的状态解算陀螺仪数据，减少错误(相对于使用 MCU 以一个低采样率的方式采样计算)
 * DMP_FEATURE_6X_LP_QUAT: 使用 DMP 以 200Hz 的频率产生 陀螺仪/加速度 四元数 .
 它不能与前面的 DMP_FEATURE_LP_QUAT 同时使用
 * DMP_FEATURE_TAP: 检测 X, Y, 和 Z 轴.
 * DMP_FEATURE_ANDROID_ORIENT: 谷歌屏幕翻转算法. 当屏幕翻转时，在四个方向产生一个事件
 * DMP_FEATURE_GYRO_CAL: 若 8s 内都没有运动，计算陀螺仪的数据
 * DMP_FEATURE_SEND_RAW_ACCEL: 添加原始 加速度 数据到 FIFO.
 * DMP_FEATURE_SEND_RAW_GYRO: 添加原始 陀螺仪 数据到 FIFO.
	* DMP_FEATURE_SEND_CAL_GYRO: 添加 校准后的 陀螺仪 数据到 FIFO.
 ...
 */
     ...
     */
		 ...
		 /*
		 启用时，DMP将以200Hz采样传感器数据，并以DMP_set_FIFO_rate API中指定的速率输出到FIFO.
		 一旦将样本放入FIFO，DMP将发送中断。因此，如果dmp_set_fifo_rate为25Hz，则MPU设备将出现25Hz中断。
		存在一个已知问题，即如果不启用DMP_FEATURE_TAP，则即使fifo速率设置为不同的速率，中断也将
		处于200Hz。为避免此问题，包括DMP_FEATURE_TAP，DMP传感器融合仅适用于+-2000dps和acce的陀螺仪
		*/
  ...
  while(1){
    ...
    if (!hal.sensors || !hal.new_gyro) {
        ...
        if (new_data) {
            inv_execute_on_data();//函数更新所有数据及状态
             ...
             */
            read_from_mpl(); //调用 read_from_mpl 函数向主机输出最新的数据。
        }
    }
